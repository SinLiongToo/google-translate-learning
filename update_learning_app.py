...
<body class="min-h-screen font-sans antialiased selection:bg-brand-500 selection:text-white">
  ...
  <main class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
    ...
    <section id="view-flashcard" class="space-y-6">
      ...
      <!-- 3D 翻轉卡片本體 (極速響應) -->
      <div class="max-w-2xl mx-auto h-[390px] perspective-1000 cursor-pointer select-none" onclick="flipCard()">
        <div id="card-inner" class="relative w-full h-full transform-style-3d shadow-xl rounded-2xl">
          
          <!-- 卡牌正面 (原文面) -->
          <div class="absolute inset-0 w-full h-full rounded-2xl p-8 flex flex-col justify-between backface-hidden bg-white dark:bg-slate-900/95 border border-slate-200 dark:border-slate-700/60 shadow-lg">
            ...
            <div class="text-center my-auto px-4">
              <div id="fc-front-lang-hint" class="text-xs font-semibold text-slate-400 uppercase tracking-widest mb-2">
                [🇫🇷 法文原文]
              </div>
              <h2 id="fc-front-text" class="field-en text-2xl sm:text-3xl font-extrabold text-slate-900 dark:text-white tracking-tight break-words leading-snug">
                donner de la confiture aux cochons
              </h2>
              <div id="fc-hint" class="text-xs text-slate-400 mt-4">
                （點擊卡牌或按空格鍵翻轉查看釋義）
              </div>
            </div>

            <!-- 底部提示 -->
            <div class="flex items-center justify-between text-xs text-slate-500 dark:text-slate-400 border-t border-slate-200 dark:border-slate-800/80 pt-3">
          ...
          <!-- 卡牌背面 (釋義與說明面) -->
          <div class="absolute inset-0 w-full h-full rounded-2xl p-8 flex flex-col justify-between backface-hidden rotate-y-180 bg-slate-50 dark:bg-slate-900/95 border border-emerald-500/40 shadow-lg">
            <div class="flex items-center justify-between">
            ...
            <!-- 背面中文/英文/法文釋義與例句內容 -->
            <div class="my-auto px-2 overflow-y-auto max-h-[220px]">
              <h3 id="fc-back-text" class="field-zh text-2xl sm:text-3xl font-bold text-emerald-700 dark:text-emerald-300 leading-relaxed">
                give jam to the pigs.
              </h3>
              <div id="fc-notes-container" class="field-notes mt-4 p-3.5 rounded-xl bg-white dark:bg-slate-800/60 border border-slate-200 dark:border-slate-700/50 text-sm text-slate-700 dark:text-slate-300 shadow-sm">
                <p id="fc-notes" class="leading-relaxed">
                  備註與例句解析
                </p>
              </div>
            </div>

            <div class="text-center text-xs text-slate-500 dark:text-slate-400 border-t border-slate-200 dark:border-slate-800/80 pt-3">
              點擊再次翻轉回正面
            </div>
          </div>
